...
class SourceCustomerIndex extends React.Component {
    ...
    createTable = () => {
        ...
        for (let i=0; i < this.state.sources.length; i++) {
            ...
            urlUpdate = '/admin/customers/source/update/' + this.state.sources[i]['id'];
            urlDelete = '/api/admin/customer_sources/delete/' + this.state.sources[i]['id'];
    ...
    render() {
        return (
            <div className="container-admin">
                <Link to="/admin/customers" className="btn peach-gradient">Назад</Link>
                ...
                <Link to="/admin/customers/source/create" className="btn btn-outline-secondary">Создать источник</Link>
            </div>
        );
    }
}
